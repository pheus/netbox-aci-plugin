# SPDX-FileCopyrightText: 2026 Martin Hauser
#
# SPDX-License-Identifier: GPL-3.0-or-later

"""Layout composition tests for the access-policies domain's detail views.

Pins which panels each view renders, in which order, and in which
column, plus the depth of its breadcrumb trail. Without this a panel
can be dropped from a layout and vanish from the page while every
other test stays green, because the panel tests instantiate their
subject directly and the inherited detail-page test asserts only the
response status.

The trailing PluginContentPanel in every column is part of the pin: it
is appended only by SimpleLayout, so its presence is what keeps other
plugins' template extensions rendering on these pages.
"""

from __future__ import annotations

from django.test import SimpleTestCase

from ....views.access_policies.aaep import (
    ACIAAEPDomainBindingView,
    ACIAttachableAccessEntityProfileView,
)
from ....views.access_policies.domains import (
    ACIPhysicalDomainView,
    ACIRoutedDomainView,
)
from ....views.access_policies.interface_policy_groups import (
    ACILeafInterfacePolicyGroupView,
)
from ....views.access_policies.leaf_interface_overrides import (
    ACILeafInterfaceOverrideView,
)
from ....views.access_policies.leaf_interface_profiles import (
    ACILeafInterfaceProfileView,
    ACILeafInterfaceSelectorView,
    ACILeafPortBlockView,
)
from ....views.access_policies.leaf_switch_profiles import (
    ACILeafNodeBlockView,
    ACILeafSelectorView,
    ACILeafSwitchProfileInterfaceBindingView,
    ACILeafSwitchProfileView,
)
from ....views.access_policies.vlan_pools import (
    ACIVLANPoolRangeView,
    ACIVLANPoolView,
)
from ..base import layout_views

# view: (left column, right column, bottom row, breadcrumb count)
EXPECTED_LAYOUTS = {
    ACIAttachableAccessEntityProfileView: (
        [
            "ACIAttachableAccessEntityProfilePanel",
            "CustomFieldsPanel",
            "PluginContentPanel",
        ],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACIAAEPDomainBindingView: (
        ["ACIAAEPDomainBindingPanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        2,
    ),
    ACIPhysicalDomainView: (
        ["ACIPhysicalDomainPanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACIRoutedDomainView: (
        ["ACIRoutedDomainPanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACILeafInterfacePolicyGroupView: (
        [
            "ACILeafInterfacePolicyGroupPanel",
            "CustomFieldsPanel",
            "PluginContentPanel",
        ],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACILeafInterfaceOverrideView: (
        ["ACILeafInterfaceOverridePanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        4,
    ),
    ACILeafInterfaceProfileView: (
        ["ACILeafInterfaceProfilePanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACILeafInterfaceSelectorView: (
        ["ACILeafInterfaceSelectorPanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        2,
    ),
    ACILeafPortBlockView: (
        [
            "ACILeafPortBlockPanel",
            "ACILeafPortBlockRangePanel",
            "CustomFieldsPanel",
            "PluginContentPanel",
        ],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        3,
    ),
    ACILeafSwitchProfileView: (
        ["ACILeafSwitchProfilePanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACILeafSelectorView: (
        ["ACILeafSelectorPanel", "CustomFieldsPanel", "PluginContentPanel"],
        [
            "ContextTablePanel",
            "TagsPanel",
            "CommentsPanel",
            "PluginContentPanel",
        ],
        ["PluginContentPanel"],
        2,
    ),
    ACILeafNodeBlockView: (
        [
            "ACILeafNodeBlockPanel",
            "ACILeafNodeBlockRangePanel",
            "CustomFieldsPanel",
            "PluginContentPanel",
        ],
        [
            "ContextTablePanel",
            "TagsPanel",
            "CommentsPanel",
            "PluginContentPanel",
        ],
        ["PluginContentPanel"],
        3,
    ),
    ACILeafSwitchProfileInterfaceBindingView: (
        [
            "ACILeafSwitchProfileInterfaceBindingPanel",
            "CustomFieldsPanel",
            "PluginContentPanel",
        ],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACIVLANPoolView: (
        ["ACIVLANPoolPanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        1,
    ),
    ACIVLANPoolRangeView: (
        ["ACIVLANPoolRangePanel", "CustomFieldsPanel", "PluginContentPanel"],
        ["TagsPanel", "CommentsPanel", "PluginContentPanel"],
        ["PluginContentPanel"],
        2,
    ),
}


def panel_names(column) -> list[str]:
    """Return the class names of the panels in a layout column."""
    return [type(panel).__name__ for panel in column]


class AccessPoliciesLayoutCompositionTestCase(SimpleTestCase):
    """Pin the panel composition of every access-policies detail view."""

    def test_layouts_render_the_expected_panels_per_column(self) -> None:
        """Each view keeps its panels, their order, and their column."""
        for view_class, expected in EXPECTED_LAYOUTS.items():
            left, right, bottom, breadcrumbs = expected
            with self.subTest(view=view_class.__name__):
                rows = list(view_class.layout)
                self.assertEqual(len(rows), 2)
                top_columns = list(rows[0])
                self.assertEqual(len(top_columns), 2)
                self.assertEqual(panel_names(top_columns[0]), left)
                self.assertEqual(panel_names(top_columns[1]), right)
                bottom_columns = list(rows[1])
                self.assertEqual(len(bottom_columns), 1)
                self.assertEqual(panel_names(bottom_columns[0]), bottom)
                self.assertEqual(len(view_class.layout.breadcrumbs), breadcrumbs)

    def test_every_layout_view_is_pinned(self) -> None:
        """A view gaining a layout without a pin fails here.

        The test above iterates EXPECTED_LAYOUTS, so its completeness is
        a property of the text unless something asserts it.
        """
        self.assertCountEqual(
            EXPECTED_LAYOUTS.keys(),
            layout_views("netbox_aci_plugin.views.access_policies"),
        )
